...
import { EVERYDAY_DISCOVERY_ITEMS } from "../app/everyday-discovery-data.ts";

const run = promisify(execFile);
const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const WORK_ROOT = join(ROOT, "work", "everyday-discovery-audio");
const MANIFEST_PATH = join(WORK_ROOT, "manifest.json");
const STANDARD_ROOT = join(ROOT, "public", "everyday-discovery-audio-standard");
const CHILD_ROOT = join(ROOT, "public", "everyday-discovery-audio");
const MODEL = "gemini-2.5-pro-preview-tts";
const VOICE = "Aoede";
const PROMPT_VERSION = "everyday-word-aoede-us-v1";
...
const DISCOVERY_WORDS = EVERYDAY_DISCOVERY_ITEMS.map((item) => ({
...
function usage() {
  return [
    "Generate prepared Aoede pronunciation audio for the Everyday Discovery Lab.",
    "",
    "Usage:",
    "  node scripts/generate-everyday-discovery-audio.mjs",
    "  node scripts/generate-everyday-discovery-audio.mjs --batch",
    "  node scripts/generate-everyday-discovery-audio.mjs --submit-batches",
    "  node scripts/generate-everyday-discovery-audio.mjs --direct",
    "  node scripts/generate-everyday-discovery-audio.mjs --dry-run",
    "",
...
function promptFor(job) {
  if (job.kind === "whole") {
    return [
      "Say the supplied English word as a warm, patient Primary 1 teacher.",
      ...
      "Say only the target word, exactly once. Do not add an article, definition, spelling, introduction, or closing sound.",
      "TARGET WORD START",
      job.name,
      "TARGET WORD END",
    ].join("\n");
  ...
  if (job.kind === "coach") return [
    "Read the supplied pronunciation-coach script as a warm, patient Primary 1 teacher.",
    "Use clear, natural General American English pronunciation.",
    `The whole word follows the classroom pronunciation ${job.pronunciationGuide}.`,
    "Pause clearly between the short sound parts, then say the whole word smoothly.",
  ...
  if (job.kind === "description") return [
    "Read the supplied three-sentence everyday-English description as a warm, engaging Primary 1 nonfiction narrator.",
  ...
  return [
    "Say the supplied sound part as a warm, patient Primary 1 English teacher.",
    "Use clear, natural General American English pronunciation.",
    `This sound part comes from the word ${job.name}, pronounced ${job.pronunciationGuide}.`,
    "Say only the sound cue, exactly once. Do not say its spelling, the whole word, an introduction, or a closing sound.",
    "TARGET SOUND START",
    job.spokenText,
    "TARGET SOUND END",
  ].join("\n");
...
function planJobs() {
  const jobs = DISCOVERY_WORDS.flatMap((word) => [
    { ...word, kind: "whole", spokenText: word.name, fileName: "whole.mp3" },
    { ...word, kind: "coach", spokenText: word.coachText, fileName: "coach.mp3" },
    { ...word, kind: "description", spokenText: word.descriptionText, fileName: "description.mp3" },
    ...word.chunks.map((chunk) => ({
      ...word,
      kind: `chunk-${chunk.id}`,
    ...
    const planned = {
      ...job,
      id,
      standardPath: `everyday-discovery-audio-standard/${job.id}/${job.fileName}`,
      childPath: `everyday-discovery-audio/${job.id}/${job.fileName}`,
    };
    planned.promptSha256 = sha256(promptFor(planned));
    planned.requestSha256 = sha256(stableJson(requestFor(planned)));
    return planned;
  });
  const expected = DISCOVERY_WORDS.reduce((total, word) => total + 3 + word.chunks.length, 0);
  if (jobs.length !== expected || new Set(jobs.map((job) => job.id)).size !== jobs.length) {
    throw new Error("Everyday discovery plan is incomplete or contains duplicate IDs.");
  }
  for (const job of jobs) {
    if (!/^[a-z0-9-]+\/[a-z0-9-]+\/(?:whole|coach|description|chunk-[a-z0-9-]+)$/.test(job.id)) throw new Error(`Unsafe job ID: ${job.id}`);
  }
  return jobs;
}
...
function batchRequestBody(jobs, planSha256) {
  return {
    batch: {
      displayName: `story-garden-everyday-discovery-${planSha256.slice(0, 12)}`,
      inputConfig: {
        requests: {
...
async function defaultManifest(jobs) {
  return {
    ...
    childTransform: { version: "everyday-child-atempo-v1", atempo: CHILD_TEMPO },
    createdAt: new Date().toISOString(),
...
async function loadManifest(jobs) {
  ...
  if (
    ...
    throw new Error("Existing Everyday Discovery audio manifest has an incompatible identity.");
  }
...
async function installDecoded(job, decoded, source) {
  const temporaryRoot = await mkdtemp(join(tmpdir(), "story-garden-everyday-discovery-"));
  const standardPath = join(ROOT, "public", job.standardPath);
  const childPath = join(ROOT, "public", job.childPath);
  try {
...
async function main() {
  ...
  missing = jobs.filter((job) => !records.has(job.id));
  if (missing.length) throw new Error(`${missing.length} everyday discovery clip(s) remain unfinished.`);
